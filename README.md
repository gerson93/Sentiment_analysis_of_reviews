...
With our preprocessed text, we'll create two word count vectors: the first with unigram where by values will be word counts, and the second with unigram and bigram where by values will be binary counts. Also, we'll create two tf-idf vectors in the same way as the previous.
